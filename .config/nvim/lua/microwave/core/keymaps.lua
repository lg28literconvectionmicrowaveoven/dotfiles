...
map('i', '<C-bs>', '<C-o>dvb', { desc = "Ctrl + Backspace deletes a word behind the cursor" }) -- Cursor current character is also deleted. Only do so smartly (when current character is space?)
map('i', '<C-Del>', '<C-o>dw', { desc = "Ctrl + Backspace deletes a word in front of the cursor" })
...
map('n', '<leader>fc', '<cmd>Telescope grep_string<cr>', { desc = "Find string under cursor in current working directory" })
map('n', '<leader>ft', '<cmd>TodoTelescope<cr>', { desc = "Find TODO comments in cwd" })
...
map('n', '<leader>ws', '<cmd>SessionSave<cr>', { desc = "Save session for current working directory (into Neovim root)" })
